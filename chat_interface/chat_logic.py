...
async def process_user_input(user_input: str | None) -> None:
    ...
    try:
        ...
        user_id = (st.session_state.get("user_name") or "").strip() or "anonymous"
        session_id = (st.session_state.get("session_id") or "").strip() or "default"

        # Set user/session in OTel context so Phoenix receives them on every span.
        # OpenInference instrumentors (OpenAI, MCP) read this and add session.id + user.id
        # to all spans in this trace before export to the server.
        with using_attributes(
            ...
            await _process_user_input_impl(user_input)
    except Exception as e:
...
async def _process_user_input_impl(user_input: str) -> None:
    """Inner implementation of chat processing, run inside using_attributes context."""
